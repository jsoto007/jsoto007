<h1 align="center">👋 Hi, I'm <span style="color:#0074E0;">Jose J. Soto</span></h1>
<h3 align="center">Full-Stack Software Engineer • SaaS Builder • Automation Developer</h3>

<p align="center">
  <img 
    src="https://readme-typing-svg.demolab.com?font=Inter&weight=500&size=20&duration=2800&pause=1200&color=0074E0&center=true&vCenter=true&width=520&lines=Building+AI-driven+automation+for+modern+businesses;Turning+complex+workflows+into+elegant+software;SaaS+that+scales+with+clarity+and+precision"
    alt="Animated Intro Line"
  />
</p>

<p align="center">
  📍 Based in <strong>NYC</strong> • Focused on clarity, automation, and elegant UX.
</p>

### ⚡ About Me

I’m a **full-stack engineer** building automation-first SaaS platforms for modern businesses.  
My work blends **React**, **Flask**, and **API-driven design** to craft user-centered software that feels effortless.  
Special passion for **workflow optimization**, **real-time data**, and **clean, scalable UI**.

---

### 🚀 What I’m Building

#### <a href="https://www.carmonitr.com" target="_blank" rel="noopener noreferrer">Carmonitr</a>
**Dealership inventory automation** — VIN scanning, location tracking, and vehicle lifecycle management.  
💳 Integrated: Stripe API  
**Stack:** React + Vite • Flask • SQLAlchemy • PostgreSQL • Render  

#### <a href="https://bravostaqueria.com" target="_blank" rel="noopener noreferrer">Bravos Taqueria</a>
Ordering + inventory with Square checkout, SMS/email automation, and live dashboard.  
**Stack:** React • Flask • Square Payments API • Mailgun  

#### <a href="https://staffmonitr.com" target="_blank" rel="noopener noreferrer">StaffMonitr</a>
Staff Matrix & Role Scheduling Automation.  
**Stack:** React • Flask • PostgreSQL • RBAC  

#### <a href="https://blackworknyc.com" target="_blank" rel="noopener noreferrer">Blackwork NYC</a>
Fine-line tattoo studio portfolio + booking automation.  
**Stack:** React • Flask • PostgreSQL • Stripe • Mailgun  

---

### 🧰 Tech Stack

**Frontend:**  
React • Vite • TypeScript • JavaScript • TailwindCSS • HTML • CSS • Framer Motion  

**Backend:**  
Python • Flask • SQLAlchemy • Ruby on Rails • PostgreSQL • REST APIs  

**Payments & APIs:**  
Stripe API • Square Payments API • Mailgun  

**DevOps & Cloud:**  
Render • Docker • Nginx • Gunicorn • GitHub Actions  

**Expertise:**  
SaaS Architecture • Multi-Tenant Systems • Automation • Clean API Design • Modern UX  

---

### 🌐 Let’s Connect

📁 **Portfolio:** [sotodev.com](https://sotodev.com)  
✉️ **Email:** [jsoto@sotodev.com](mailto:jsoto@sotodev.com)  
💼 **LinkedIn:** [linkedin.com/in/jose-javier-soto-a29b96111](https://www.linkedin.com/in/jose-javier-soto-a29b96111)

---

⭐ _I love creating user-centric solutions and making things more efficient._

